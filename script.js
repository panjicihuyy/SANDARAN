...
/* ---- Payment method & kembalian (kalkulator kembalian tunai) ---- */
function parseCashInput(raw){
  const digits = (raw || "").replace(/[^0-9]/g, "");
  return digits ? parseInt(digits, 10) : 0;
}
function formatCashInputValue(n){
...
function updateCashCalc(){
  const cashInput = $("#cashGivenInput");
  const resultBox = $("#cashResult");
  const resultValue = $("#cashResultValue");
  const resultLabel = $("#cashResultLabel");
  if(!cashInput || !resultBox || !resultValue || !resultLabel) return;

  const total = cartTotal();
  const cash = parseCashInput(cashInput.value);
  const change = cash - total;

  if(cash <= 0){
    resultBox.classList.remove("is-insufficient");
    resultLabel.textContent = "Kembalian";
    resultValue.textContent = rupiah(0);
    return;
  }

  if(change < 0){
    resultBox.classList.add("is-insufficient");
    resultLabel.textContent = "Kurang";
    resultValue.textContent = rupiah(Math.abs(change));
  } else {
    resultBox.classList.remove("is-insufficient");
    resultLabel.textContent = "Kembalian";
    resultValue.textContent = rupiah(change);
  }
}

function toggleCashCalcVisibility(){
  const cashCalc = $("#cashCalc");
  if(!cashCalc) return;
  const isCash = getSelectedPaymentMethod() === "cash";
  cashCalc.hidden = !isCash;
  if(isCash) updateCashCalc();
}

function initPaymentMethod(){
  $$('input[name="paymentMethod"]').forEach(r => r.addEventListener("change", toggleCashCalcVisibility));

  const cashInput = $("#cashGivenInput");
  cashInput?.addEventListener("input", () => {
    cashInput.value = formatCashInputValue(parseCashInput(cashInput.value));
    updateCashCalc();
  });

  $$("[data-cash-quick]").forEach(btn => {
    btn.addEventListener("click", () => {
      const val = btn.dataset.cashQuick;
      const target = $("#cashGivenInput");
      if(!target) return;
      target.value = val === "exact" ? formatCashInputValue(cartTotal()) : formatCashInputValue(parseInt(val, 10));
      updateCashCalc();
    });
  });

  toggleCashCalcVisibility();
}
initPaymentMethod();
...
checkoutFormEl.addEventListener("submit", (e) => {
  ...
  if(paymentMethod === "cash"){
    cashGiven = parseCashInput($("#cashGivenInput")?.value);
    if(cashGiven <= 0){
      showToast("Isi dulu nominal uang tunai yang kamu bawa ya 🙏");
      $("#cashGivenInput")?.focus();
      return;
    }
    if(cashGiven < total){
      showToast("Uang tunainya belum cukup untuk bayar total pesanan");
      $("#cashGivenInput")?.focus();
      return;
    }
  ...
  const order = {
    ...
    payment: paymentMethod === "cash"
      ? `Tunai (COD) — dibayar ${rupiah(cashGiven)}`
      : "Transfer / QRIS / E-wallet — dikonfirmasi via WhatsApp",
    cashGiven,
    change,
    customer: {
...
function finalizeOrder(order){
  ...
  const changeCard = $("#successChangeCard");
  if(changeCard){
    if(order.paymentMethod === "cash" && order.change > 0){
      $("#successChangeValue").textContent = rupiah(order.change);
...
$("#shareOrderBtn")?.addEventListener("click", () => {
  if(!lastOrder) return;
  const itemsText = lastOrder.items.map(it => `- ${it.name}${it.sub ? " ("+it.sub+")" : ""} x${it.qty}`).join("\n");
  const paymentText = lastOrder.paymentMethod === "cash"
    ? `Tunai (COD) — dibawa ${rupiah(lastOrder.cashGiven)}, kembalian ${rupiah(lastOrder.change)}`
    : "Transfer / QRIS / E-wallet";
  const msg = `Halo Sandaran! 👋 Saya baru saja pesan lewat website:\n\n` +
    `No. Pesanan: ${lastOrder.id}\n` +
    `Nama: ${lastOrder.customer?.name || "-"}\n` +
    `${itemsText}\n` +
...
function buildReceiptHTML(order){
  ...
  const cashRows = order.paymentMethod === "cash" ? `
      <div class="r-row"><span>Uang Dibayar</span><span>${rupiah(order.cashGiven)}</span></div>
...
/* =========================================================
   ...
   ========================================================= */
let particleColorRGB = "60,70,48"; // dibaca oleh sistem partikel, diperbarui saat tema berganti
const THEME_STORAGE_KEY = "sandaran_theme";
...
function applyTheme(theme, { animate = true } = {}){
  ...
  if(animate){
    ...
    themeTransitionTimer = setTimeout(() => root.classList.remove("theme-transition"), 650);
  }
  ...
  if(metaTheme) metaTheme.setAttribute("content", theme === "dark" ? "#17140F" : "#3C4630");

  try{ localStorage.setItem(THEME_STORAGE_KEY, theme); }catch(err){ /* ignore */ }
...
function updateParticleColor(theme){
  particleColorRGB = theme === "dark" ? "230,222,201" : "60,70,48";
}
...
function initParticles(){
  ...
  function densityForWidth(w){
    // roughly one particle per ~16,000 px² of viewport, capped for perf
    return Math.min(90, Math.max(28, Math.round((w * height) / 16000)));
  }
  ...
  function tick(){
    ...
    particles.forEach(p => {
      p.x += p.vx + Math.sin(t * 0.4 + p.driftSeed) * 0.08;
      p.y += p.vy;
      ...
      ctx.fillStyle = `rgba(${particleColorRGB}, ${p.alpha})`;
      ctx.fill();
    });
    requestAnimationFrame(tick);
  }
